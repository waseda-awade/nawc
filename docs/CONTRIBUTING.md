...
3. **Scores** (if any), in any tabular form, with whatever links them to texts/writers.
...
1. Convert your texts to corpus format with a **scripted, logged pipeline**
   (`tools/intake_convert.py`) — no manual copy-paste. SHA-256 checksums of your received
   files are recorded in a private processing log.
...
complete `data/<subcorpus-name>/` directory (see `standard/` for the schema and validator;
